  ...
  try {
    bcrypt.hash(req.body.mdp, saltRounds).then(function (hashPW) {
      if (req.body.statut === 'mentor') {
        const sql = `INSERT INTO mentor (nom_mentor, prenom_mentor, mail_mentor, mdp_mentor, photo_mentor, nom_SIREN, statut_mentor) VALUES ('${req.body.nom}',
         '${req.body.prenom}', '${req.body.mail}', '${hashPW}', '${req.body.photo}', '${req.body.nom_SIREN}', false)`;
        connection.query(sql)
        res.status(201).send('vos coordonnées comme mentor sont inscrits')
      } else if (req.body.statut === 'apprenti') {
        const sql = `INSERT INTO apprenti (nom_apprenti, prenom_apprenti, mail_apprenti, mdp_apprenti, photo_apprenti) VALUES ('${req.body.nom}',
         '${req.body.prenom}', '${req.body.mail}', '${hashPW}', '${req.body.photo}')`;
        connection.query(sql)
